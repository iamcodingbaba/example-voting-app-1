        ...
        stage("docker build and push"){
            steps{
                sh '''
                    ...
                    sudo docker push 271919715952.dkr.ecr.us-east-1.amazonaws.com/demo:latest-${BUILD_NUMBER}
                    sudo docker stop $(docker ps -a -q)
                    sudo docker rm $(docker ps -a -q)
                    sudo docker run -itd -p 627:80 271919715952.dkr.ecr.us-east-1.amazonaws.com/demo:latest-${BUILD_NUMBER}
